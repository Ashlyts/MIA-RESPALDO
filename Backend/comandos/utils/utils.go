...
func EscribirMBR(file *os.File, mbr *structures.MBR) error {
	if _, err := file.Seek(0, 0); err != nil {
		return err
	}
...
// TieneTypeMKFS valida el tipo de formateo
func TieneTypeMKFS(typeParam string) string {
	typeUpper := strings.ToUpper(strings.TrimSpace(typeParam))

	if typeUpper == "" || typeUpper == "FULL" {
		return "FULL"
	}

	return "FULL" // Por defecto siempre FULL
}
